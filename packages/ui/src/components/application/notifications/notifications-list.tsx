...
const NotificationItem: FC<NotificationItemProps> = ({ title, description, date, unread, user, icon: Icon, actions, variant = "base" }) => {
    ...
    return (
        <div className={cx("relative flex gap-3 p-4", unread && "bg-brand-primary_alt")}>
            ...
            {/* Leading element */}
            {variant === "avatar" && user && (
                <Avatar src={user.avatarUrl} alt={user.name} size="md" className="shrink-0" />
            )}
            {variant === "icon" && (
                <div className="shrink-0">
                    {Icon ? <Icon className="size-5 text-fg-secondary" /> : <DefaultIcon className="size-5 text-fg-secondary" />}
                </div>
            )}
            {variant === "featured-icon" && (
                <FeaturedIcon icon={Icon ?? DefaultIcon} color="gray" theme="modern" size="sm" className="shrink-0" />
            )}
                    ...
                    <div className="mt-2 flex gap-3">
                        {actions.map((action) => (
                            <Button
                                key={action.label}
                                size="sm"
                                color={action.color === "brand" ? "link-color" : "link-gray"}
