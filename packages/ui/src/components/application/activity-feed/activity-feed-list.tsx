...
const ActivityFeedItem: FC<ActivityFeedItemProps> = ({
  user,
  action,
  target,
  targetHref,
  ...
  return (
    <article className="relative flex gap-3">
      {unseen && <span className="absolute top-1 -left-4 size-2 rounded-full bg-fg-success-secondary" />}
    ...
    <div className={cx("flex flex-col", isDivided && "divide-y divide-secondary", !isDivided && !isConnected && "gap-6", className)}>
      {items.map((item, index) => (
        <div key={item.id} className={cx(isDivided && "py-4 first:pt-0 last:pb-0")}>
          <ActivityFeedItem {...item} showConnector={isConnected && index !== items.length - 1} variant={variant} />
        </div>
      ))}
    </div>
  );
