...
const Connector = ({ type, size }: { type: "line" | "end" | "none"; size: "sm" | "md" }) => (
    <div className="relative size-4 shrink-0" data-tree-connector>
        {type === "line" && (
            <div
                className={cx("absolute top-1/2 left-1/2 w-px -translate-x-1/2 -translate-y-1/2 bg-border-secondary", size === "sm" ? "h-10.5" : "h-11")}
            />
        )}
        {type === "end" && (
            <div className={cx("absolute bottom-2 left-1/2 w-px -translate-x-1/2", size === "sm" ? "h-5.5" : "h-6")}>
                <div
                    className={cx(
                        ...
                        size === "sm" ? "h-5.5 w-2.5 rounded-bl-[5px]" : "h-6 w-3 rounded-bl-md",
                    )}
                />
            </div>
        )}
