...
import { BellRinging01, Calendar as CalendarIcon, ChevronLeft, ChevronRight, Clock, Copy01, Edit01, Trash01 } from "@opus2-platform/icons";
import {
  ...
  const calculateEventSpan = useCallback(
    (event: ZonedEvent, currentDate: CalendarDate) => {
      ...
      if (!startsOnThisDay && eventStartDay.compare(currentDate) < 0) {
        ...
        const isFirstDayOfWeek = weekStartDate ? isSameDay(currentDate, weekStartDate) : false;
    ...
    const findAvailableRow = (startDayIndex: number, span: number): number => {
      ...
      while (true) {
        ...
        for (let offset = 0; offset < span; offset++) {
          ...
          const occ = occupancy[row] as Set<string> | undefined;
          if (occ && occ.size > 0) {
            isAvailable = false;
            break;
          }
            ...
            <div className="flex flex-col gap-3">
              <section className="flex gap-2">
                <section className="flex flex-row -space-x-2">
                  <Avatar className="ring-[1.5px] ring-bg-primary" src={codexDemoAvatar("cal-sienna")} alt="Sienna Hewitt" size="sm" />
                  <Avatar className="ring-[1.5px] ring-bg-primary" src={codexDemoAvatar("cal-ammar")} alt="Ammar Foley" size="sm" />
                  <Avatar className="ring-[1.5px] ring-bg-primary" src={codexDemoAvatar("cal-pippa")} alt="Pippa Wilkinson" size="sm" />
                  <Avatar className="ring-[1.5px] ring-bg-primary" src={codexDemoAvatar("cal-olly")} alt="Olly Schroeder" size="sm" />
                  <Avatar className="ring-[1.5px] ring-bg-primary" src={codexDemoAvatar("cal-mathilde")} alt="Mathilde Lewis" size="sm" />
              ...
              <section className="flex items-center gap-2">
                ...
                <span className="h-3.25 border-l border-primary" />
  ...
  return (
    <div
      ...
      className={cx(
        ...
        view === "month" ? "h-full md:min-h-228" : "h-228",
        className
      )}
    >
      <CalendarHeader
