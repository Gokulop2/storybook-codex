...
export const NavList = ({ activeUrl, items, className, dualTier }: NavListProps) => {
  ...
  const mapMobileRow = (item: NavItemType | NavItemDividerType, index: number) => {
    ...
    return (
      <li key={nav.label} className="py-px">
        <NavItemBase type="link" badge={nav.badge} icon={nav.icon} href={nav.href ?? "#"} current={activeUrl === nav.href}>
          {nav.label}
        </NavItemBase>
      </li>
    );
  ...
  const mapDesktopFlatRow = (item: NavItemType | NavItemDividerType, index: number) => {
    ...
    if (nav.items?.length) {
      return (
        <li key={nav.label} className="py-px">
          <NavItemBase type="link" badge={nav.badge} icon={nav.icon} href={parentHref(nav)} current={isBranchCurrent(nav, activeUrl)}>
            {nav.label}
          </NavItemBase>
        </li>
      );
  ...
  if (dualTier) {
    return (
      <>
        <ul data-nav-list className={cx("flex list-none flex-col px-4 pt-5 lg:hidden", className)}>
          {items.map(mapMobileRow)}
        </ul>
        <ul data-nav-list className="mt-5 hidden list-none flex-col px-4 lg:flex">
          {items.map(mapDesktopFlatRow)}
        </ul>
      </>
    );
  }
  ...
  return (
    <ul data-nav-list className={cx("flex list-none flex-col px-4 pt-5", className)}>
      {items.map(mapMobileRow)}
    </ul>
  );
};
