...
import { ArrowLeft, ArrowRight, ChevronLeft, ChevronLeftDouble, ChevronRight, ChevronRightDouble } from "@opus2-platform/icons";
  ...
  return (
    <Pagination.Item
      ...
      className={({ isSelected }) =>
        cx(
          "flex size-9 cursor-pointer items-center justify-center p-3 text-sm font-medium text-quaternary outline-hidden transition duration-100 ease-linear hover:bg-primary_hover hover:text-secondary focus-visible:z-10 focus-visible:bg-primary_hover focus-visible:outline-2 focus-visible:outline-offset-2",
...
const MobilePagination = ({ page = 1, total = 10, className, onPageChange }: MobilePaginationProps) => {
  return (
    <nav aria-label="Pagination" className={cx("flex items-center justify-between md:hidden", className)}>
      <Button aria-label="Go to previous page" iconLeading={ArrowLeft} color="secondary" size="sm" onClick={() => onPageChange?.(Math.max(0, page - 1))} />

      <span className="text-sm text-fg-secondary">
        Page <span className="font-medium">{page}</span> of <span className="font-medium">{total}</span>
      </span>

      <Button aria-label="Go to next page" iconLeading={ArrowRight} color="secondary" size="sm" onClick={() => onPageChange?.(Math.min(total, page + 1))} />
    </nav>
  );
};
  ...
  return (
    <div className={cx("border-t border-secondary px-4 py-3 md:px-6 md:pt-3 md:pb-4", className)}>
      ...
      <nav aria-label="Pagination" className={cx("hidden items-center gap-3 md:flex", align === "center" && "justify-between")}>
        ...
        <div className={cx("flex items-center gap-3", align === "right" && "order-first mr-auto", align === "left" && "order-last ml-auto flex-row-reverse")}>
              ...
              {pages.map((page, index) =>
                page.type === "page" ? (
                  ...
                  <Pagination.Ellipsis key={index}>
                    <ButtonGroupItem className="pointer-events-none size-9 items-center justify-center rounded-none!">&#8230;</ButtonGroupItem>
                  </Pagination.Ellipsis>
                )
              )}
...
export const PaginationCardAdvanced = ({
  ...
  return (
    <div className={cx("border-t border-secondary px-4 py-3 md:px-6 md:pt-3 md:pb-4", className)}>
      <Pagination.Root page={page} total={total} onPageChange={onPageChange} className={cx("flex items-center gap-3", align === "center" && "justify-between")}>
