      ...
      {!isSelectionEmpty &&
        tagSelectContext?.selectedItems?.items?.map((value) => (
          ...
          >
            ...
            <p className={cx("truncate font-medium whitespace-nowrap text-secondary select-none", size === "sm" ? "ml-1 text-xs" : "ml-1.25 text-sm")}>
      ...
      {({ isDisabled }) => (
        <>
          ...
          <FocusScope contain={false} autoFocus={false} restoreFocus={false}>
            <InnerTagSelect isDisabled={isDisabled} size={size} shortcut={shortcut} shortcutClassName={shortcutClassName} placeholder={placeholder} />
          </FocusScope>
        </>
      )}
    </AriaGroup>
