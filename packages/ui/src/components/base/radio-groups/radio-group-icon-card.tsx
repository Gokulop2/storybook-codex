      ...
      {items.map((plan) => (
        <AriaRadio
          ...
          className={({ isDisabled, isSelected, isFocusVisible }) =>
            cx(
              "relative block cursor-pointer rounded-xl bg-primary outline-hidden ring-inset",
          ...
          {({ isDisabled, isSelected, isFocusVisible }) => (
            <>
              ...
              >
                ...
                <span className={cx("mr-1 text-secondary", size === "md" ? "text-lg font-semibold" : "text-md font-semibold")}>{plan.title}</span>
              ...
              <div className="flex flex-col-reverse items-start justify-between gap-4 rounded-b-lg p-4 sm:flex-row sm:gap-1">
                <div className={cx("flex flex-col", size === "md" ? "gap-2" : "gap-1")}>
                  <p className="flex items-baseline gap-1">
                    <span className={cx("text-secondary", size === "md" ? "text-display-md font-semibold" : "text-display-sm font-semibold")}>
                      {plan.price}
                    </span>
