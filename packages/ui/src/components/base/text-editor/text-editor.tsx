...
const TextEditorComposed = ({
  ...
  const inputCls = cx(layout === "hero" ? "min-h-[320px]" : size === "sm" ? "min-h-[120px]" : "min-h-[160px]");

  return (
    <TextEditorRoot {...rootProps} limit={maxLength} content={defaultContent} inputClassName={inputCls} className={className}>
      {showToolbar && <TextEditorToolbar floating={floatingToolbar} />}
      {showTooltips && <TextEditorTooltip />}
      <TextEditorContent />
      {(hintText || maxLength) && <TextEditorHintText>{hintText}</TextEditorHintText>}
    </TextEditorRoot>
  );
};
