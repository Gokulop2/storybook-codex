...
const DocsSection: FC<DocsSectionProps> = ({ id, title, code, description, sectionClassName, dataPreview, previewClassName, children }) => {
  ...
  const panelClassName = [previewClassName ?? CHECKBOX_PREVIEW_SURFACE_CLASS, DOCS_PREVIEW_P_MARGIN_RESET, isPreviewDark && "dark-mode"].filter(Boolean).join(" ");
  ...
  <div className="bg-primary min-h-screen font-sans" data-codex-docs-checkboxes>
    ...
    <main className="relative mx-auto flex w-full max-w-304 min-w-0 flex-1 flex-col">
      <div className="text-tertiary w-full min-w-0">
        ...
        <DocsSection id="base" title="Base" code={CODE.base}>
          <div className="flex items-start">
            <Checkbox aria-label="Checkbox" />
          </div>
        </DocsSection>

        <DocsSection id="with-label" title="With label" code={CODE.withLabel}>
          <div className="flex items-start">
            <Checkbox label="Remember me" />
          </div>
        </DocsSection>

        <DocsSection id="with-label-and-hint" title="With label and hint" code={CODE.withLabelHint}>
          <div className="flex items-start">
            <Checkbox label="Remember me" hint="Save my login details for next time." />
          </div>
        </DocsSection>

        <DocsSection id="disabled" title="Disabled" code={CODE.disabled}>
          <div className="flex items-start">
            <Checkbox label="Remember me" hint="Save my login details for next time." isDisabled />
          </div>
        </DocsSection>

        <DocsSection id="sizes" title="Sizes" code={CODE.sizes}>
